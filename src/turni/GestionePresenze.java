		...
		modificaPresenza.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				if (tabellaPresenze.getSelectedRow() >= 0)
					new SetPresenza((PresenzaLavorativa) vectorToPresenza(tabellaPresenze.getSelectedRow()),
							tabellaPresenze.getSelectedRow());
			}
		});
		...
		while (iterator.hasNext()) {
			...
			if (presenza instanceof PresenzaLavorativa) {
				...
				if (p.getImpianto().equals(i))
					temp.add(aggiungiPresenza(p.getIdentificativo(), p.getDescrizione(), p.getInizio().toString(),
							p.getFine().toString(), p.getPausa().toString(), p.impegno().toString()));
			}
		}
		return temp;
	}

	private static Vector<String> aggiungiPresenza(String identificativo, String descrizione, String oraInizio,
			String oraFine, String pausa, String impegno) {
		Vector<String> temp = new Vector<String>();
	...
	class EliminaPresenza implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			if (tabellaPresenze.getSelectedRow() >= 0) {
				...
				int conferma = JOptionPane.showConfirmDialog(null, "Sei Sicuro?", "Attesa Conferma",
